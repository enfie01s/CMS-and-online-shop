...
function divdisplay(divId,linkId)
{
	theDiv = document.getElementById(divId);
	if(theDiv != null)
	{
		theLink = document.getElementById(linkId);
		theDiv.style.display = (theDiv.style.display == "none") ? "inline" : "none";
		theLink.innerHTML = (theDiv.style.display == "none") ? "hide "+divId : "show "+divId;
	}
}
function brochure(theURL,scrollbars)
{
 LeftPosition=(screen.width)?(screen.width-1000)/2:100;
 TopPosition=(screen.height)?(screen.height-700)/2:100;
 winName='';
 settings = 'width='+1000+', height='+700+', top='+TopPosition+', left='+LeftPosition+', scrollbars='+scrollbars+', location=no, directories=no, status=0, menubar=no, toolbar=no, resizable=no';
 URL = 'content/ebrochure/index.php?b='+theURL;
 window.open(URL,winName,settings);
}
function ajaxStuff(subject,theObj)
{
	if(theObj.value.length>3)
		ajax(subject,theObj);
}
function showhideSug(onoff)
{
	document.getElementById('suggestbox').style.display=onoff==0?'none':'block';
}
function warrantyLength(br,prod)
{
	theField=document.forms['warranty_reg']['skuvariant[358]'];
	mod=0;
	if(br=="FRANCHI"&&prod.length>0)
	{
		mod=prod.search("ALCIONE")!=-1?3:(prod.search("AFFINITY")!=-1?7:0);
	}
	//var brandSku={WARRANTY3:'BERETTA',WARRANTY10:'BERETTA',WARRANTY5:'BENELLI'};
	//theSelected=theField[theField.selectedIndex].id.replace("-v-NONE","");
	//if(brandSku[theSelected]==br)
	
	theField.children['WARRANTY3-v-NONE'].disabled=br=="BERETTA"||(br=="FRANCHI"&&mod==3)?false:true;//3 year option
	theField.children['WARRANTY7-v-NONE'].disabled=br=="FRANCHI"&&mod==7?false:true;//7 year option
	theField.children['WARRANTY10-v-NONE'].disabled=br=="BERETTA"?false:true;//10 year option
	theField.children['WARRANTY5-v-NONE'].disabled=br=="BENELLI"?false:true;
	theField.children['WARRANTY1-v-NONE'].disabled=br=="STOEGER"?false:true;
	theField.children['WARRANTY2-v-NONE'].disabled=br=="SAKO"||br=="TIKKA"?false:true;
	//theField.children['WARRANTY7-v-NONE'].disabled=br=="FRANCHI"?false:true;

	cLen=theField.children.length;
	for(p=0;p<cLen;p++)
	{
		if(theField[p].disabled==true)
			theField[p].selected=false;
	}
}
...
function blankForm(formObj)
{
	theForm = document.getElementById(formObj);
	theLength = theForm.elements.length;
	for(x=0;x<theLength;x++)
	{
		if(theForm.elements[x].name != "p" && theForm.elements[x].type != "submit" && theForm.elements[x].type != "reset")
		{
			switch(theForm.elements[x].type)
			{
				case "select-one"://dropdowns
					theForm.elements[x].selectedIndex=0;
					break;
				case "checkbox"://check boxes
					theForm.elements[x].checked=false;
					break;
				case "radio"://radios
					if(theForm.elements[x].value == "all")
						theForm.elements[x].checked=true;
					else
						theForm.elements[x].checked=false;
					break;
				default://text
					theForm.elements[x].value='';//text boxes
					break;
			}
		}
	}
...
function newsarticle(a,b,id,whichaxis)
{
	slide(a,b,id,whichaxis);
}
slideTimer = 0;
function slide(a,b,id,whichaxis)
{
	if(a > b)
	{
		a = (a-5 > b) ? a-5 : a-(a-b);
		switch(whichaxis) 
		{
			case "top":
				...
				break;
			case "right":
				document.getElementById(id).style.right = a+"px";
				break;
			case "bottom":
		...
		{
			slideTimer = setTimeout("slide("+a+","+b+",'"+id+"','"+whichaxis+"')",0);
		}
	}
	else
	{
...
overlay.style.height=document.documentElement.scrollHeight+500+'px';
