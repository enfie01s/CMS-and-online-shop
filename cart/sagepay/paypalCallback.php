...
if (($_REQUEST["navigate"]=="proceed")) 
{
    ...
    else
    {
        ...
        /*$strSQL = "SELECT Amount FROM cart_rders WHERE `VPSTxId` = '" . mysql_real_escape_string($strVPSTxId) . "'";
        $rsPrimary = ysql_query($strSQL) or die ("Query '$strSQL' failed with error message: \"" . mysql_error () . '"');
		if ($row = mysql_fetch_array($rsPrimary))*/
		$strSQL = "SELECT Amount FROM cart_orders WHERE `VPSTxId` = ?";
        $rsPrimary = $db1->prepare($strSQL);
				$rsPrimary->execute(array($strVPSTxId));
		if ($row = $rsPrimary->fetch())
		{
...
else
{
    ...
    else
    {
        // Using the VPSTxId we can retrieve our order transaction amount from our database 
        // This ensures we have the correct amount and its a valid existing order.
        /*$strSQL = "SELECT * FROM cart_orders WHERE `VPSTxId` = '" . mysql_real_escape_string($strVPSTxId) . "'";
        $rsPrimary = ysql_query($strSQL) or die ("Query '$strSQL' failed with error message: \"" . mysql_error () . '"');
		if ($row = mysql_fetch_array($rsPrimary))*/
		$strSQL = "SELECT * FROM cart_orders WHERE `VPSTxId` = ?";
    $rsPrimary = $db1->prepare($strSQL);
		$rsPrimary->execute(array($strVPSTxId));
		if ($row = $rsPrimary->fetch())
		{
	...
	** Registration results come back in the Status and StatusDetail fields */
    ...
    if ($GLOBALS["strStatus"] == "FAIL")   
    {
	   $GLOBALS["strPageError"]="An Error has occurred whilst trying to post the PayPal completion request to Sage Pay.<BR>
		...
		ysql_query($strSQL) or die ("Query '$strSQL' failed with error message: \"" . mysql_error () . '"');*/
$strSQL="UPDATE cart_orders SET `Status`='? - PAYPAL COMPLETION FAILED: ?' WHERE `VPSTxId`=?";
		
		$q=$db1->prepare($strSQL);
		$q->execute(array($GLOBALS["strStatus"],$GLOBALS["strStatusDetail"],$GLOBALS["strVPSTxId"]));
	}
    else
    {
		...
		/*$strSQL="UPDATE cart_orders SET `Status`='" .  mysql_real_escape_string($GLOBALS["strStatus"]) . " - " . mysql_real_escape_string($GLOBALS["strStatusDetail"]) . "'";
		...
		ysql_query($strSQL) or die ("Query '$strSQL' failed with error message: \"" . mysql_error () . '"');*/
		$binds=array();
		$strSQL="UPDATE cart_orders SET `Status`=?";$binds[]=$GLOBALS["strStatus"]. " - " .$GLOBALS["strStatusDetail"];
if (strlen($GLOBALS["strTxAuthNo"])>0) 
		{ 
			$strSQL.= ",`TxAuthNo`=?";$binds[]=$GLOBALS["strTxAuthNo"];
		}
		$strSQL.= " WHERE `VPSTxId`=?";$binds[]=$GLOBALS["strVPSTxId"];
		$q=$db1->prepare($strSQL);
		$q->execute($binds);
...
{    
    ...
    $strSQL=$strSQL . " where `VPSTxId`='" . mysql_real_escape_string($GLOBALS["strVPSTxId"]) . "'";
	return ysql_query($strSQL) or die ("Query '$strSQL' failed with error message: \"" . mysql_error () . '"');
}*/
function updateDatabase()
{    
	$binds=array();
	$strSQL="UPDATE tblOrders set `Status`=?";$binds[]=$GLOBALS["strStatus"] . " - " . $GLOBALS["strStatusDetail"];
	if (strlen($GLOBALS["strAddressStatus"])>0){ $strSQL.= ",`AddressStatus`=?";$binds[]=$GLOBALS["strAddressStatus"];}
	if (strlen($GLOBALS["strPayerStatus"])>0){ $strSQL.= ",`PayerStatus`=?";$binds[]=$GLOBALS["strPayerStatus"];}
	if (strlen($GLOBALS["strPayerID"])>0) {$strSQL.= ",`PayPalPayerID`=?";$binds[]=$GLOBALS["strPayerID"];}
	if (strlen($GLOBALS["strDeliveryFirstnames"])>0) {$strSQL.= ",`DeliveryFirstnames`=?";$binds[]=$GLOBALS["strDeliveryFirstnames"];}
	if (strlen($GLOBALS["strDeliverySurname"])>0) {$strSQL.= ",`DeliverySurname`=?";$binds[]=$GLOBALS["strDeliverySurname"];}
	if (strlen($GLOBALS["strDeliveryAddress1"])>0){ $strSQL.= ",`DeliveryAddress1`=?";$binds[]=$GLOBALS["strDeliveryAddress1"];}
	if (strlen($GLOBALS["strDeliveryAddress2"])>0) {$strSQL.= ",`DeliveryAddress2`=?";$binds[]=$GLOBALS["strDeliveryAddress2"];}
	if (strlen($GLOBALS["strDeliveryCity"])>0) {$strSQL.= ",`DeliveryCity`=?";$binds[]=$GLOBALS["strDeliveryCity"];}
	if (strlen($GLOBALS["strDeliveryPostCode"])>0) {$strSQL.= ",`DeliveryPostCode`=?";$binds[]=$GLOBALS["strDeliveryPostCode"];}
	if (strlen($GLOBALS["strDeliveryCountry"])>0) {$strSQL.= ",`DeliveryCountry`=?";$binds[]=$GLOBALS["strDeliveryCountry"];}
	if (strlen($GLOBALS["strDeliveryState"])>0) {$strSQL.= ",`DeliveryState`=?";$binds[]=$GLOBALS["strDeliveryState"];}
	if (strlen($GLOBALS["strDeliveryPhone"])>0) {$strSQL.= ",`DeliveryPhone`=?";$binds[]=$GLOBALS["strDeliveryPhone"];}
	if (strlen($GLOBALS["strCustomerEMail"])>0) {$strSQL.= ",`CustomerEMail`=?";$binds[]=$GLOBALS["strCustomerEMail"];}
    $strSQL.= " where `VPSTxId`=?";$binds[]=$GLOBALS["strVPSTxId"];
	$q=$db1->prepare($strSQL);
...
<body>
    <div id="pageContainer">
        <?php include "../../stella-test/sagepay/header.html"; ?>
        <?php include "../../stella-test/sagepay/resourceBar.html"; ?>
        <div id="content">
				...
				<tr>
					...
					<td class="fieldData">
						<table width="100%" style="border-collapse: collapse;">
							...
							$rsPrimary = ysql_query($strSQL) or die ();
																			
							while ($row = mysql_fetch_array($rsPrimary)) */
							$strSQL="SELECT op.`Price`,op.`Quantity`,p.* FROM tblOrderProducts op
									inner join tblProducts p on op.`ProductId`=p.`ProductId`
									where op.`VendorTxCode`=?";
							
							$rsPrimary = $db1->prepare($strSQL);
							$rsPrimary->execute(array($strVendorTxCode));	
							while ($row = $rsPrimary->fetch()) 
							{
